<?php
    ...
    class Penjualan_online_model extends CI_Model 
    {
        ...
        {
            $this->db->select('*, RIGHT(penjualan_online.id_transaksi,2) as kode,');
			$this->db->from('penjualan_online');
			$this->db->join('pelanggan', 'pelanggan.id = penjualan_online.id_pelanggan');
			$this->db->order_by('kode', 'desc');
			return $this->db->get()->result();
        }
        public function getHPP($id){
			$this->db->select('sum((a.jumlah)*b.harga_produk_satuan) as nominal');
			$this->db->from('logcart a');
        ...
        public function readDetailPenjualan($id)
        {
			...
			$this->db->select('*,logcart.id as id , barang.nama_barang, barang.id as id_barang');
			$this->db->from('logcart');
