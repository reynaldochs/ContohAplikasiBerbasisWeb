           ...
           <div class="cart-view-table">
             <form  action="<?= site_url()?>/cart/do_checkout" method="post">
               <div class="table-responsive">
                      ...
                      <div>
                        <?php
                        $tgl_max = date('Y-m-d', strtotime('+1 days', strtotime($data->tanggal_transaksi)));
                        if(date('Y-m-d') > $tgl_max){?>
                        <p>*Pembayaran telah expired</p>
                        <a href="<?= site_url('user/order/bayar/'.$data->id_transaksi)?>" class="btn aa-browse-btn" disabled style="margin-left: 80%">Expired</a>
                        <?php }else{
                        if ($data->status==2) { ?>
                        <p>*Lakukan pembayaran sebelum <?= date('d F Y',strtotime($tgl_max))?></p>
                        <a href="<?= site_url('user/order/bayar/'.$data->id_transaksi)?>" class="btn aa-browse-btn" style="margin-left: 80%">Bayar Sekarang</a>
                        <?php  }elseif ($data->status==3) {                         
                        ?>
                        <a href="#" class="btn btn-success" style="margin-left: 80%">Waitng For Confirmation</a>
                      <?php }elseif ($data->status==4) { ?>
                        <a href="#" class="btn btn-info" style="margin-left: 80%">Order Confirmed</a>
                      <?php }elseif($data->status==6){ ?>
                        <div class="row">
                          <div class="col-sm-6">
                          No Resi : <?= $data->no_resi?>
                          </div>
                          <a href="<?= base_url() ?>upload/BuktiKirim/<?= $data->bukti_resi ?>"  target="blank" class="btn btn-info" style="margin-left: 35%">Cek Bukti Resi</a>
                        </div>
                      <?php }
                      } ?>
                        <br>
