...
class cart extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['produk'] = $this->Cart_model->read($where,"cart.id DESC");
    ...
    function addcart($id_brg){
        ...
        $dataStore = array('pelanggan_id' => $this->session->userdata("id_user"),
                            ...
                            'jumlah'    => 1,
                            'status' => 1 );
        $this->db->where(array('pelanggan_id' => $this->session->userdata("id_user"), 'barang_id' => $id_brg, 'status <='=>2));
    ...
    function checkout(){
        ...
        foreach ($cart as $datas) {            
            ...
            if($dt[0]->status == 2){
                $insert = [
                    ...
                    $this->db->where('id_cart',$datas->id);
                    $this->db->set($insert);
        ...
        $data['produk'] = $this->Cart_model->readlog(array('pelanggan_id' => $this->session->userdata("id_user"),'status' => 2),"logcart.id DESC");
        // $data['view'] = "user/home";
    ...
    function do_checkout(){
        ...
        foreach ($produk as $datas) {            
            ...
            $this->db->set(array('status' => 3, 'id_transaksi'=>$this->input->post('id_transaksi')));
            $this->db->update('logcart');
        }
